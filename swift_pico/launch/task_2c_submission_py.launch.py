...
def generate_launch_description():
    ...
    pico_server = Node(
        package='swift_pico',
        executable='pico_server_2c.py',
        name='pico_server_2c'

    )
    pico_cilent = Node(
        package='swift_pico',
        executable='pico_client_2c.py',
        name='pico_client_2c'

    )
    warehouse_service = Node(
        package='swift_pico',
        executable='warehouse_service.py',
        name='warehouse_service'

    )

    rosbag = launch.actions.ExecuteProcess(
            cmd=['ros2', 'bag', 'record', '-o', 'task_2c', '/whycon/poses','/package_loc','/rosin'],
            output='screen'
        )
