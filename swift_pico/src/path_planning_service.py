...
class Path(Node):
    ...
    def waypoint_callback(self, request, response):
        ...
        if request.get_path == True:
            ...
            response.waypoints.data.append(self.waypoints[2][0])
            response.waypoints.data.append(self.waypoints[2][1])
            for waypoint in self.waypoints:
                if grid[waypoint[0]][waypoint[1]] == 0:
                    print(f"Waypoint {waypoint} in black")
            self.get_logger().info("Incoming request for Waypoints") 
            self.get_logger().info(f"Waypoints: {self.waypoints[0]}, {self.waypoints[1]}, {self.waypoints[2]}")
            self.path_1 = a_star_search(self.waypoints[0], self.waypoints[1]) 
            self.path_2 = a_star_search(self.waypoints[1], self.waypoints[2])   
            if self.path_1:
                ...
                self.get_logger().info("Path 1 output")
            else:
                self.get_logger().error("No path1 found")
            if self.path_2:
                ...
                self.get_logger().info("Path 2 output")
            else:
                self.get_logger().error("No path2 found")
            return response
        else:
