...
class MovingAverage:
    ...
    def exponential_filter(self, raw_error, prev_error):
        if abs(raw_error) >= abs(prev_error):
            new_value = 0.3 * raw_error + 0.7 * prev_error
        else:
            new_value = 0.7 * raw_error + 0.3 * prev_error
        return new_value
...
class WayPointServer(Node):
    ...
    def __init__(self):
        ...
        self.Kp = np.array([10.0, 10.0, 12.0, 3.0])
        self.Ki = np.array([0.01, 0.01, 0.085, 0.05])
        self.Kd = np.array([350.0, 350.0, 430.0, 150.0])
    ...
    def pid(self):
        ...
        raw_error = self.drone_position[2] - self.setpoint[2]
        self.error[2] = MovingAverage().exponential_filter(raw_error, self.prev_error[2])
        ...
        [self.out_roll, self.out_pitch, self.out_throttle, self.out_yaw] = self.controlled_error
        self.values = np.array([-self.out_roll, self.out_pitch, self.out_throttle, self.out_yaw]) + 1500
        for i in range(4):
        ...
        self.cmd.rc_yaw = int(self.values[3])
    ...
    def execute_callback(self, goal_handle):
        ...
        while True:
            ...
            if self.is_reached_waypoint(self.setpoint):
                i += 5
                if i < len(self.path[0])+5:
                    self.setpoint = self.path[0][i]
                    print(f"WP next: {i}: {self.setpoint}")
                else:
                    print("Completed Path1.")
                    break
            last_point_1 = len(self.path[0])-(len(self.path[0])-1)%5
            
            
            # Final destination check
            if np.sum(self.setpoint == self.path[0][last_point_1]) == 4 or np.sum(self.setpoint == self.path[1][-1]) == 4:
