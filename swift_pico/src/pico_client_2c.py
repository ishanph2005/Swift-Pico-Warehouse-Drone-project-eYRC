...
class WayPointClient(Node):
    ...
    def __init__(self):
        ...
        self._action_client = ActionClient(self, WaypointPathNew, 'waypoint_path_new')
        
        # Service client for the service 'GetWaypoints' with service name 'waypoints'
        self.cli = self.create_client(GetPathNew, 'path_new')
        ...
        self.request = GetPathNew.Request()
    ...
    def send_goal(self):
        ...
        for point in self.full_path:
            ...
            pathway.z = point[2]
            pathway.theta = 0.0
            path_msg.path.append(pathway)  # Append the populated Pathway object to path1
        print("Goal sent")
    ...
    def receive_goals(self):
        ...
        points = Pathway()   
        for points in response.path:
            
            imgx = points.x
            imgy = points.y
            goal_next = self.pixel_to_whycon(imgx, imgy) 
            self.full_path.append(goal_next)
        self.get_logger().info('Path received by the action client')
        self.send_goal()
