...
class Swift_Pico(Node):
    def __init__(self):
        ...
        self.setpoint = np.array([2.0, 2.0, 20.0])  # [x_setpoint, y_setpoint, z_setpoint]
        ...
        self.scale_kp = np.array([1.0, 1.0, 1.0])       # Scale factors
        self.scale_ki = np.array([1.0, 1.0, 1.0])
        self.scale_kd = np.array([1.0, 1.0, 1.0])
    ...
    def arm(self):
        ...
        self.command_pub.publish(self.cmd)  # Publishing /drone_command
        self.get_logger().info("Drone armed! Ready to accept commands")
